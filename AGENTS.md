...
- **Outputs:** `output/channel_video_audit.csv` (Output 1) and `output/review_report.csv` / `output/review_report.html` (Output 2, current run only). Each Tool 2 run also writes `output/combined_review_report.csv` / `.html`, which merge the cache to cover *every* video with a transcript. Generated files are never committed.
